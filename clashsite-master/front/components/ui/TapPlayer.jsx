...
const BUILDER_HERO_NAMES = new Set(["Battle Machine", "Battle Copter"]);
...
const ICON_OVERRIDES = {
  ...
  "p.e.k.k.a": ["troops/Icon_HV_P.E.K.K.A.png", "troops/P.E.K.K.A.png"],
  "super yeti": [
  ...
  "overgrowth spell": ["spells/Icon_HV_Dark_Spell_Overgrowth.png"],
  "ice block spell": [
...
const pickIcon = (iconUrls) => {
  if (!iconUrls) return "";
  return (
    iconUrls.medium || iconUrls.large || iconUrls.small || iconUrls.tiny || ""
  );
};
...
const buildIconCandidates = (category, item) => {
  ...
  const villageKey = (item?.village || "home").toLowerCase().includes("builder")
    ? "builder"
    : "home";
  let mappingCategory = category;
  if (category === "troops") {
    if (PET_NAMES.has(rawName) || PET_NAMES.has(normalizedName))
      mappingCategory = "pets";
    else if (SIEGE_NAMES.has(rawName) || SIEGE_NAMES.has(normalizedName))
      mappingCategory = "siege";
  ...
  const alias =
    aliasMap[normalizedName] || aliasMap[decodedName] || aliasMap[rawName];
  if (alias) {
  ...
  if (category === "equipment") {
    ...
    heroes.forEach((code) =>
      pushLocal(`equipment/Hero_Equipment_${code}_${slug}.png`)
    );
...
const UnitCard = ({ item, category }) => {
  const candidates = useMemo(
    () => buildIconCandidates(category, item),
    [category, item]
  );
  ...
  return (
    <section className="space-y-3">
      ...
      <div className="flex flex-wrap gap-3 rounded-2xl bg-slate-900/70 px-4 py-5">
        {items.map((item) => (
          <UnitCard
            key={`${item.name}-${item.level ?? item.stars ?? 0}`}
            item={item}
            category={category}
          />
        ))}
            ...
            >
              ...
              <div className="flex flex-wrap gap-4">
                {group.items.map((item) => (
                  <UnitCard
                    key={`${group.hero}-${item.name}-${item.level}`}
                    item={item}
                    category="equipment"
                  />
                ))}
      ...
      {other.length ? (
        <div className="rounded-2xl bg-slate-900/70 p-4 ring-1 ring-slate-800/60 shadow-inner">
          ...
          <div className="flex flex-wrap gap-4">
            {other.map((item) => (
              <UnitCard
                key={`other-${item.name}-${item.level}`}
                item={item}
                category="equipment"
              />
            ))}
...
const buildTroopGroups = (player) => {
  ...
  const homeTroops = troops.filter(
    (unit) => (unit.village || "home") === "home"
  );
  const builderTroops = troops.filter(
    (unit) => (unit.village || "home") === "builderBase"
  );
  ...
  const homeSuper = homeTroops.filter(
    (unit) => unit.name?.startsWith("Super ") || unit.superTroopIsActive
  );
  const homeRegular = homeTroops.filter(
    (unit) =>
      !PET_NAMES.has(unit.name) &&
      !SIEGE_NAMES.has(unit.name) &&
      !homeSuper.includes(unit)
  );

  const builderHeroes = heroes.filter((hero) =>
    BUILDER_HERO_NAMES.has(hero.name)
  );
  const builderRegularHeroes = heroes.filter(
    (hero) =>
      (hero.village || "home") === "builderBase" &&
      !BUILDER_HERO_NAMES.has(hero.name)
  );
  ...
  return {
    ...
    builder: [
      ...
      {
        title: "Builder Heroes",
        items: builderRegularHeroes,
        category: "heroes",
      },
...
const buildSpellGroups = (player) => {
  ...
  return {
    home: [
      {
        title: "Spells",
        items: spells.filter((spell) => (spell.village || "home") === "home"),
        category: "spells",
      },
    ].filter((group) => group.items.length > 0),
    builder: [
      {
        title: "Builder Spells",
        items: spells.filter(
          (spell) => (spell.village || "home") === "builderBase"
        ),
        category: "spells",
      },
    ].filter((group) => group.items.length > 0),
...
const buildHeroGroups = (player) => {
  ...
  return {
    home: [
      {
        title: "Heroes",
        items: heroes.filter((hero) => (hero.village || "home") === "home"),
        category: "heroes",
      },
    ].filter((group) => group.items.length > 0),
    builder: [
      {
        title: "Builder Heroes",
        items: heroes.filter(
          (hero) => (hero.village || "home") === "builderBase"
        ),
        category: "heroes",
      },
    ].filter((group) => group.items.length > 0),
...
const buildAchievementGroups = (player) => {
  ...
  return {
    home: [
      {
        ...
        items: achievements.filter(
          (ach) => ach.village === "home" || ach.village === "homeVillage"
        ),
    ...
    builder: [
      {
        title: "Builder Base",
        items: achievements.filter((ach) => ach.village === "builderBase"),
        category: "achievements",
      },
    ].filter((group) => group.items.length > 0),
  };
...
const equipmentSynonym = (name) => {
  if (!name) return name;
  if (/^Henchmen\s*Puppet$/i.test(name) || /^Henchman\s*Puppet$/i.test(name))
    return "Henchman";
  if (/^Hog\s*Rider\s*Puppet$/i.test(name) || /^HogRider\s*Puppet$/i.test(name))
    return "Hog Rider Doll";
  if (/^(WWE\s*)?Action\s*Figure$/i.test(name) || /^ActionFigure$/i.test(name))
    return "WWEActionFigure";
...
export default function PlayerCollections({ player, section }) {
  const buildGroups = useMemo(
    () => SECTION_BUILDERS[section] || (() => ({ home: [], builder: [] })),
    [section]
  );
  ...
  const heroEquipmentData = useMemo(
    () =>
      buildHeroEquipmentAssignments(
  ...
  const groupsToRender =
    activeVillage === "builder" ? sectionGroups.builder : sectionGroups.home;
  ...
  const renderContent = () => {
    if (section === "achievements") {
      return (
        <AchievementGrid
          items={groupsToRender.flatMap((group) => group.items)}
        />
      );
    }
